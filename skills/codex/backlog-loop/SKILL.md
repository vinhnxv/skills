...
- Marker present: an earlier run of this procedure claimed it and did not finish. Read its notes first. A `merge requested: <pr-url>` note means that run may already have shipped it, so query that PR: `MERGED` -> finish the interrupted close, appending the calibration note and closing against that URL. Anything else -> reclaim with `bd update <id> --status=open --assignee="" --unset-metadata backlog_loop_run --append-notes="reclaimed from interrupted run <old-run-id>"`, returning it to `bd ready`.
- Marker absent: externally owned. Never steal it. If no ready work exists and one of these remains, stop and report that the backlog is not clear. A failed or blocked attempted batch is marked `blocked`, so it cannot be selected again silently. CI is batch-scoped and re-resolved from the exact current trunk SHA every iteration; it may change in either direction.
